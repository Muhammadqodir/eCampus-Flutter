...
class CacheSystem{
  ...
  static Future<bool> isActualCache()async{
    final prefs = await SharedPreferences.getInstance();
    String cacheDateStr = prefs.getString("${prefix}date")??"2001-08-06 10:45:00";
    DateFormat dateFormat = DateFormat("yyyy-MM-dd HH:mm:ss");
  ...
  static void saveStudentCache(StudentCache cache) async{
    ...
    if(cache.userPic!=null){
      await prefs.setString('${prefix}userPic', base64Encode(cache.userPic!.toList()));
    }
    await prefs.setString("${prefix}rating", jsonEncode(cache.ratingModel.toJson()));
